...
def crear_contador(inicio):
    ...
    def realizar_ventas(**datos_venta):
        ...
        try:
                ...
                for clave, valor in productos_registrados.items():
                    ...
                    if clave.lower() == datos_venta["Producto"].lower():
                        ...
                        if valor["Stock"] < datos_venta["Cantidad"]:
                            ...
                            print(
                                f"No hay esa cantidad de productos en {clave}. En stock tenemos {valor["Stock"]}"
                            )
                        ...
                        # Se ejecuta en caso de que si haya las unidades solicitadas
                            ...
                            registro_ventas.update(
                                {
                                    contador: {
                                        "Producto": datos_venta["Producto"],
                                        "Cantidad": datos_venta["Cantidad"],
                                        "Total": (
                                            [precio_total * 0.8, " Con descuento"]
                                            if precio_total > 15000
                                            else precio_total
                                        ),
                            ...
                            return (
                                ...
                                datos_venta["Cantidad"],
                                (
                                    str(precio_total * 0.8)
                                    + " Se le aplico un descuento del 20%"
                                    if precio_total > 15000
                                    else precio_total
                                ),
                                datos_venta["Cliente"],
            ...
            print(
                "Por favor ingresa bien los datos. Estamos esperando datos tipo **kwargs (clave - valor)",
                e,
            )
...
if __name__ == "__main__":
    ...
    try:
        contador, nombre, cantidad, total, cliente, fecha, hora = operacion(
            Producto="Leche", Cantidad=7, Cliente="Daniela Carolina"
        )
        print(
            f"""Venta realizada:
